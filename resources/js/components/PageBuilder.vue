            ...
            <button type="button"
              ...
              :class="[
                saveSuccess 
                  ...
                  : 'bg-primary-600 hover:bg-primary-700 active:bg-primary-800 focus:border-primary-800 focus:ring-primary-200'
              ]"
            >
    ...
    try {
      ...
      if (props.record && props.record.id) {
        ...
        const pageData = {
          ...
          _token: document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') // Incluir CSRF token
        }
        
        // Faz a requisição para o servidor usando POST em vez de PUT
        ...
        console.warn('PageBuilder - Não foi possível salvar no servidor: ID do registro não encontrado')
      ...
      alert('Erro ao salvar a página: ' + (error.message || 'Erro desconhecido'))
    } finally {
